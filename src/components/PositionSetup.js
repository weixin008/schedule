...
const PositionSetup = () => {
  ...
  const loadData = () => {
    ...
    const tagsData = localStorageService.getCustomTags() || [];
  ...
  const savePositions = (data) => {
    localStorageService.savePositions(data);
    loadData();
  };
  ...
  const handlePersonnelAssign = (targetKeys) => {
    const newData = positions.map(position => 
      position.id === selectedPosition.id 
        ? { ...position, assignedPersonnel: targetKeys }
        : position
    );
    savePositions(newData);
  ...
  const getAvailablePersonnel = (requiredTags) => {
    return personnel.filter(person => 
      person.status === '在岗' && 
      (requiredTags.includes('全部') || requiredTags.includes(person.tag))
    );
  };
  ...
  const positionColumns = [
    {
      ...
      render: (text) => (
       ...
       render: (tags) => (
         <Space wrap>
           {tags && tags.map(tag => (
             <Tag key={tag} color={tag === '领导' ? 'red' : tag === '职工' ? 'blue' : tag === '全部' ? 'purple' : 'green'}>
               {tag}
             </Tag>
           ))}
         </Space>
  ...
  return (
    <div style={{ padding: '24px', height: '100%', overflow: 'auto' }}>
                    ...
                    renderItem={(personId, index) => (
                      ...
                      >
                        <Space>
                          ...
                          <Tag size="small" color={getPersonnelTag(personId) === '领导' ? 'red' : 'blue'}>
                            {getPersonnelTag(personId)}
                          </Tag>
                          <Tag size="small" color={getPersonnelStatus(personId) === '在岗' ? 'success' : 'warning'}>
                            {getPersonnelStatus(personId)}
                          </Tag>
                        </Space>
                      </List.Item>
                    )}
      ...
      >
                 {selectedPosition && (
           <Transfer
             dataSource={getAvailablePersonnel(selectedPosition.requiredTags || []).map(person => ({
              key: person.id,
              title: `${person.name} (${person.tag})`,
              description: person.phone || ''
            }))}
            targetKeys={selectedPosition.assignedPersonnel || []}
            onChange={handlePersonnelAssign}
